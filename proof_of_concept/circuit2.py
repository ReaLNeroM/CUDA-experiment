...
"""
Implementation of .crc to CUDA converter using two copies the array.
...
class Line:
    ...
    def emit(self, variable_copy: int) -> str:
        """Emit the value of the data line at that particular time step."""
        return "{}{}[{}]".format(self.array_name, variable_copy, self.index)
...
class Circuit:
    ...
    operation_stages: List[List[Operation]]
    arg: str
    ...
    def __init__(
        ...
        # We want the number of operation stages to be even, so that the output is in the first
        # copy of the array
        self.operation_stages = [[] for _ in range(time_units_count + time_units_count % 2)]
        self.arg = arg
    ...
    def emit_lines(self) -> List[str]:
        ...
        # Stage emission
        for ind, stage in enumerate(self.operation_stages):
            # At ind = 0, x1 gets written using x0
            # At ind = 1, x0 gets written using x1
            # At ind = 2, x1 gets written using x0 etc.
            kernel_lines.append(
                "{}{}[tid] = {}{}[tid];".format(self.arg, (ind + 1) % 2, self.arg, ind % 2)
            )
            ...
            if ind + 1 != len(self.operation_stages):
